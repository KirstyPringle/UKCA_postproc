...
iris.netcdf_no_unlimited =False #DPG - removed FUTURE since these are now standard and otherwise causes an error on Jasmin saying this is deprectated
...
print input_files_directory+'um*'
pp_files=glob(input_files_directory+'um*')
...
processes=20
print 'Number of pp_names for L0', len(pp_files)
...
for chunk in list_of_chunks:
    ...
    for step_file in chunk:
        p = multiprocessing.Process(target=load_all_steps, args=(step_file,))
        print step_file,p
        jobs.append(p)
        p.start()
    for job in jobs:
